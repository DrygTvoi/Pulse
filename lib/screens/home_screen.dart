...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// We deliberately do NOT show a modal accept/decline popup for group
  /// calls — once there are several groups active at once the popups
  /// pile up on top of each other and interrupt whatever the user is
  /// reading. The chat-screen "ongoing call" banner (driven by the same
  /// sfu_invite stream via ChatController's activeGroupCalls map) is
  /// the single way to join a group call. 1-on-1 calls still popup
  /// through their separate `_listenForIncomingCalls` path.
  void _listenForIncomingGroupCalls() {
    // No-op: intentionally left empty. Registration into activeGroupCalls
    // still happens upstream in ChatController's dispatcher listener.
  }
