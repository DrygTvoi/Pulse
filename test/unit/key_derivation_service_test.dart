...
void main() {
  ...
  group('KeyDerivationService output length', () {
    test('deriveNostrKey returns exactly 32 bytes', () async {
      final key = await KeyDerivationService.deriveNostrKey('password');
      expect(key.length, equals(32));
    });

    test('deriveOxenSeed returns exactly 32 bytes', () async {
      final key = await KeyDerivationService.deriveOxenSeed('password');
      expect(key.length, equals(32));
    });
  });
  ...
  group('KeyDerivationService determinism', () {
    test('deriveNostrKey is deterministic for same password', () async {
      final k1 = await KeyDerivationService.deriveNostrKey('recovery phrase');
      final k2 = await KeyDerivationService.deriveNostrKey('recovery phrase');
      expect(k1, equals(k2));
    });

    test('deriveOxenSeed is deterministic for same password', () async {
      final k1 = await KeyDerivationService.deriveOxenSeed('recovery phrase');
      final k2 = await KeyDerivationService.deriveOxenSeed('recovery phrase');
      expect(k1, equals(k2));
    });
  });
  ...
  group('KeyDerivationService password sensitivity', () {
    test('different passwords produce different Nostr keys', () async {
      final k1 = await KeyDerivationService.deriveNostrKey('password1');
      final k2 = await KeyDerivationService.deriveNostrKey('password2');
      expect(k1, isNot(equals(k2)));
    });

    test('different passwords produce different Oxen seeds', () async {
      final k1 = await KeyDerivationService.deriveOxenSeed('password1');
      final k2 = await KeyDerivationService.deriveOxenSeed('password2');
      expect(k1, isNot(equals(k2)));
    });
  ...
  group('KeyDerivationService domain separation', () {
    test('same password yields different Nostr key and Oxen seed', () async {
      final nostrKey = await KeyDerivationService.deriveNostrKey('shared-password');
      final oxenSeed = await KeyDerivationService.deriveOxenSeed('shared-password');
  ...
  group('KeyDerivationService non-trivial output', () {
    test('Nostr key is not all-zero bytes', () async {
      final key = await KeyDerivationService.deriveNostrKey('any password');
      expect(key.any((b) => b != 0), isTrue);
    });

    test('Oxen seed is not all-zero bytes', () async {
      final seed = await KeyDerivationService.deriveOxenSeed('any password');
  ...
  group('KeyDerivationService return type', () {
    test('deriveNostrKey returns Uint8List', () async {
      final key = await KeyDerivationService.deriveNostrKey('test');
      expect(key, isA<Uint8List>());
    });

    test('deriveOxenSeed returns Uint8List', () async {
      final seed = await KeyDerivationService.deriveOxenSeed('test');
