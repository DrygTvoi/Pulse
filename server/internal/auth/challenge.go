...
const (
	challengePrefix   = "pulse-auth-v1:"
	challengePrefixV2 = "pulse-v2-auth:"
...
// The client must sign the string "pulse-auth-v1:<nonce>:<timestamp>".
...
// VerifyResponseV2 verifies an Ed25519 signature for v2 auth.
// The client must sign the string "pulse-v2-auth:<nonce>:<timestamp>".
func VerifyResponseV2(pubkeyHex string, signatureHex string, nonce string, timestamp int64) bool {
	age := time.Now().Unix() - timestamp
	if age < 0 || age > challengeMaxAge {
		return false
	}
	...
	message := challengePrefixV2 + nonce + ":" + strconv.FormatInt(timestamp, 10)
	return ed25519.Verify(ed25519.PublicKey(pubkeyBytes), []byte(message), sigBytes)
}
...
// SignChallengeV2 signs a v2 challenge for testing purposes.
func SignChallengeV2(privateKey ed25519.PrivateKey, nonce string, timestamp int64) string {
	message := challengePrefixV2 + nonce + ":" + strconv.FormatInt(timestamp, 10)
	sig := ed25519.Sign(privateKey, []byte(message))
	return hex.EncodeToString(sig)
}
